...
{asset.images.length > 1 && (
  <div className={styles.nav}>
    <button onClick={(e) => { e.stopPropagation(); setCurrentImg(prev => prev > 0 ? prev - 1 : asset.images.length - 1); }}>
      ←
    </button>
    <button onClick={(e) => { e.stopPropagation(); setCurrentImg(prev => (prev + 1) % asset.images.length); }}>
      →
    </button>
  </div>
)}
